<template>
    <div v-if="total_pages > 1" class="flex justify-between w-full">
        <p class="pl-0 py-2 rounded my-2 text-xs xs:text-sm text-gray-900">
            Showing {{ (currentPage - 1) * per_page + 1 }} to {{ currentPage * per_page }} of {{ total }} Entries
        </p>

        <ul class="flex pl-0 list-none rounded my-2">
            <li class="leading-tight bg-white border border-gray-300 border-r-0 ml-0 rounded-l hover:bg-primary-light"
            ...
            <li class="leading-tight bg-white border border-gray-300 border-r-0 hover:bg-primary-light"
                :class="{'bg-gray-200': isInFirstPage}">
                <button type="button"
                        class="py-2 px-3"
                        :class="{'cursor-not-allowed': isInFirstPage}"
            ...
            <template v-if="showDots('left')">
                <li class="leading-tight bg-white border border-gray-300 border-r-0 hover:bg-primary-light"
                    :class="{'bg-primary': isInFirstPage}">
                    <button type="button"
                            class="py-2 px-3"
                            :class="{'cursor-not-allowed': isInFirstPage}"
                            :disabled="isInFirstPage"
            ...
            <li class="leading-tight bg-white border border-gray-300 border-r-0 rounded-r border-r hover:bg-primary-light"
                :class="{'bg-gray-200': isInLastPage}">
                <button type="button"
                        class="py-2 px-3"
                        :class="{'cursor-not-allowed': isInLastPage}"
