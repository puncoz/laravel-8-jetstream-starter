# Application #
#####################################################################################
set :application,     'rem.work'
set :branch,          ENV["branch"] || "master"
set :user,            ENV["user"] || ENV["USER"] || "rem_work"


# SCM #
#####################################################################################
set :repo_url,        'git@gitlab.com:integrid.solutions/rem.work-backend.git'
set :repo_base_url,   :'https://gitlab.com'
set :repo_diff_path,  :'integrid.solutions/rem.work-backend/compare/master...'
set :repo_branch_path,:'integrid.solutions/rem.work-backend/tree'
set :repo_commit_path,:'integrid.solutions/rem.work-backend/commit'
...
# Slack Integration #
#####################################################################################
set :slack_incoming_hook_url,   "https://hooks.slack.com/services/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"
...
namespace :installation do
    ...
    task :node_modules_copy do
        on roles(:web) do
            puts ("--> Copying node_modules from previous release")
            execute "vendorDir=#{current_path}/node_modules; if [ -d $vendorDir ] || [ -h $vendorDir ]; then cp -a $vendorDir #{release_path}/node_modules; fi;"
        end
    end
    ...
    task :create_symlink do
        on roles(:app) do
            within release_path do
                ...
                execute "ln -s #{shared_path}/storage/app/public/images #{release_path}/public/images"
            end
        end
    end
...
before "deploy", "notification:start"
namespace :deploy do
